...
import { QuerySchema, ListTablesSchema, DescribeTableSchema, ListIndexesSchema, ExecutionMutationSchema, GetTableSchemaSchema, SearchDataSchema } from "./types.js";
        ...
        try {
            switch (name) {
                ...
                case "describe_table": {
                    ...
                    const columnsRes = await pool.query(`SELECT column_name, data_type, is_nullable, column_default 
             FROM information_schema.columns 
             WHERE table_schema = $1 AND table_name = $2 
             ORDER BY ordinal_position`, [schema, table]);
                    const constraintsRes = await pool.query(`SELECT constraint_name, constraint_type 
             ...
             WHERE table_schema = $1 AND table_name = $2`, [schema, table]);
                ...
                case "search_data": {
                    ...
                    const tablesRes = await pool.query("SELECT table_name FROM information_schema.tables WHERE table_schema = $1 AND table_type = 'BASE TABLE'", [schema]);
                    const results = [];
                    // Search in each table
                    for (const tableRow of tablesRes.rows) {
                        const tableName = tableRow.table_name;
                        // Get text columns for this table
                        const columnsRes = await pool.query(`SELECT column_name, data_type 
               FROM information_schema.columns 
               WHERE table_schema = $1 AND table_name = $2 
                 AND (data_type LIKE '%character%' OR data_type LIKE '%text%' OR data_type LIKE '%varchar%')`, [schema, tableName]);
                        if (columnsRes.rows.length === 0)
                            continue;
                        // Build dynamic query to search in all text columns
                        const columnConditions = columnsRes.rows.map(col => `${col.column_name}::text ILIKE $1`).join(" OR ");
                        const searchSql = `
              SELECT 
...
// --- Server Lifecycle ---
async function startHttpServer(port = 9008) {
