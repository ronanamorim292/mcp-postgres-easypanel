...
async function testTools() {
  ...
  try {
    ...
    const queryResult = await pool.query(
      "SELECT id, nome, status FROM projetos_automacao ORDER BY id LIMIT 2"
    );
    ...
    const updateResult = await pool.query(
      `UPDATE projetos_automacao SET status = 'inativo' WHERE nome = $1 RETURNING id, nome, status`,
      ['Teste MCP']
    );
    ...
    const deleteResult = await pool.query(
      `DELETE FROM projetos_automacao WHERE nome = $1 RETURNING id, nome`,
      ['Teste MCP']
    );
    ...
    const ddlColumns = await pool.query(
      `SELECT column_name, data_type, is_nullable, column_default 
       FROM information_schema.columns 
       WHERE table_schema = 'public' AND table_name = $1`,
      [tableToDescribe]
